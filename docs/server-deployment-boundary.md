...
知识库当前通过解析到私网地址 `10.47.193.5` 的内网 HTTP 域名
`qa-kb.10.47.193.5.nip.io` 提供页面和 API；入口仅面向受控内网。
...
HTTP 仅允许解析结果全部为私网或回环地址；任意公网 HTTP 仍会被后端和本地
Runner 拒绝。任务密钥由后端逐任务随机生成并只保存哈希，不得在 `.env` 中
预置、打印或提交任务密钥。
