...
## 四·补、剩下的时间花在哪：应用层，不是数据库

用「按 PID 汇总」的手法把一次检索请求的数据库耗时单独算出来（方法见第五节）：

```
热查询（嵌入已命中缓存，不触发嵌入）= 455 ms

  PID 119178  检索连接       71.2 ms / 36 条语句
  PID 119209                 4.6 ms / 24 条
  PID 105874  后台 worker     4.2 ms / 38 条
  PID 119171                 3.6 ms / 30 条
  ──────────────────────────────────────────
  数据库合计                 85.8 ms / 130 条语句
```

两个结论：

**1. 索引修复得到直接确认。** 同一个 worker（PID 105874）从修复前的
**每周期约 430 ms** 降到 **4.2 ms**。

**2. 剩下的约 370 ms 完全不在数据库里。** 455 ms 的请求，数据库只占 85.8 ms，
嵌入为 0（缓存命中），**其余约 370 ms 是 Python / ORM 的应用层开销**。

佐证：**一次检索请求在数据库侧产生约 36 条语句**（检索连接），
外加其他连接上的若干条，窗口内合计 130 条 —— 这是典型的 **N+1 查询**特征。

另外注意这些查询 `SELECT` 了 `knowledge_items` 的**全部列**，其中包含 `content`
这类大文本字段（全表 44 MB / 18,630 行，平均每行约 2.4 KB）。
按行构造 ORM 对象、反序列化大文本，代价不低。

### 下一步方向（应用层，需要代码改动）

| 方向 | 预期收益 | 说明 |
|---|---|---|
| 消除 N+1：合并/预取那 36 条语句 | 可能是最大的一块 | 需要梳理检索路径的取数逻辑 |
| 只取需要的列（避免 `content` 等大字段） | 减少 ORM 与反序列化开销 | 用 `load_only` 或显式列选择 |
| 减少连接数（4 条连接承载一次请求） | 减少往返 | 需确认是否连接池配置问题 |

这三项都属于后端代码改动，风险与验证成本都高于本次的索引与配置改动，
**需要单独排期**，不在本次范围内。
