...
答疑质检案例聚类分析 V5
改进：
1. chat_log优先：从聊天记录提取真实用户问题（core_issue不可靠）
2. 知识化标题：每个主题生成可读的知识标题 + 常见问法副标题
3. 会话联动：同一session的多条案例强制合并（Rule 6）
4. 继承V4的混合聚类策略 + 分类规则覆盖
"""
...
# ============================================================
# 0.5 去重：同一session_id的多行案例只保留最完整的一条
# ============================================================
...
# ============================================================
# 2.5 chat_log 优先：从聊天记录提取真实用户问题
# ============================================================

def get_chat_question(row):
    ...
    if not chat or chat == 'nan':
        return core
    user_q_match = re.search(r'(?:用户问题|问题描述)[：:]\s*(.+?)(?:转人工|$)', chat)
    if user_q_match:
        real_question = user_q_match.group(1).strip()
        if len(real_question) >= 8:
    ...
    for line in lines:
        ...
        if re.match(r'\d{2}/\d{2}/\d{2}\s+\d{2}:\d{2}:\d{2}', line):
            continue
        if any(skip in line for skip in ['消息类型', '质检问题', '转人工', '预览', '已加载全部']):
            continue
        if len(line) >= 10:
            return line
    return core
...
def get_combined_text(row, use_chat_priority=True):
    chat_q = get_chat_question(row)
    core = str(row['core_issue']) if not pd.isna(row['core_issue']) else ''
    ...
    if use_chat_priority and chat_q and chat_q != core:
        return chat_q + ' ' + result + ' ' + basis[:200]
    else:
        return core + ' ' + result + ' ' + basis[:200]
...
def assign_object_domain(row):
    combined = get_combined_text(row)
...
def generate_topic_title(c):
    ...
    # 简化部件名
    comp_short = comp.split('/')[0] if '/' in comp else comp

    # 模板匹配
    if std == '外观-损伤判定':
        return f'如何判定{proj_short}{comp_short}{anom}'
    elif std == '外观-结构/粘合判定':
        return f'如何判定{proj_short}{comp_short}{anom}'
    elif std == '外观-洁净判定':
        return f'如何判定{proj_short}{comp_short}{anom}'
    elif std == '外观-合规确认':
        return f'如何确认{proj_short}{comp_short}外观是否正常'
    elif std == '显示-漏液/亮点判定':
        ...
        return f'如何识别{proj_short}{comp_short}维修痕迹'
    elif std == '拆修-更换判定':
        return f'如何判定{proj_short}{comp_short}是否更换'
    elif std == '信息-型号核实':
        return f'如何确认{proj_short}机型与版本'
    elif std == '信息-序列号核实':
        return f'如何核实{proj_short}序列号'
    elif std == '信息-配置核实':
        return f'如何确认{proj_short}{comp_short}配置'
    elif std == '信息-全新机标准':
        ...
        return f'如何判定{proj_short}{comp_short}功能故障'
    elif std == '功能-声音异常判定':
        return f'如何判定{proj_short}{comp_short}异响'
    elif std == '可回收性判定':
        return f'{proj_short}{comp_short}的可回收性判定'
    else:
        # 其他判定：基于部件+异常生成
        if comp != '综合/未识别' and anom != '其他异常':
            return f'{proj_short}{comp_short}{anom}判定标准'
        else:
            return f'{proj_short}{c["object_domain"]}判定标准'


def generate_common_questions(c, df, max_n=3):
    """从聚类案例的chat_log中提取常见问法作为副标题"""
    questions = []
    seen = set()
    for idx in c['indices']:
        row = df.loc[idx]
        chat_q = get_chat_question(row)
        if chat_q and len(chat_q) >= 10 and chat_q not in seen:
            # 截断过长的文本
            short_q = chat_q[:80].strip()
            if short_q not in seen:
...
for i, c in enumerate(all_clusters):
    label = make_label(c)
    title = generate_topic_title(c)
    common_q = generate_common_questions(c, df)
    for idx in c['indices']:
        row = df.loc[idx]
        output_data.append({
            '主题编号': i + 1,
...
output_path = f'data/聚类结果_v5_{timestamp}.xlsx'
...
for i, c in enumerate(all_clusters):
    ...
    for idx in c['indices'][:5]:
        row = df.loc[idx]
        # V5: 优先使用chat_log中的真实问题作为示例
        chat_q = get_chat_question(row)
        issue_text = chat_q if chat_q and len(chat_q) > 10 else str(row['core_issue'])
        examples.append(f"[{str(row['model'])[:40]}] {issue_text[:200]}")
...
summary_path = f'data/主题摘要_v5_{timestamp}.xlsx'
...
if noise_cases:
    ...
    noise_path = f'data/噪声排除案例_v5_{timestamp}.xlsx'
    noise_df.to_excel(noise_path, index=False, engine='openpyxl')
    print(f"噪声案例: {noise_path}")

# Markdown报告
with open(f'data/聚类分析报告_v5_{timestamp}.md', 'w', encoding='utf-8') as f:
