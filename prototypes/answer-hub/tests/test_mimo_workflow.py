...
from answer_hub.clustering_rules import build_clustering_fingerprint
...
from answer_hub.workflow import (
    ...
    _failed_topic_transcription_row,
...
def _online_item(
    ...
    *,
    ...
    return CzPublishedKnowledgeItem(
        ...
        models=(),
        keywords=(),
        source_ref="knowledge-kb://knowledge/BIZ-TEST-001",
        status="published",
        version="test-v1",
        score=score,
        final_score=score,
        boundary_projection=boundary_projection or {},
    )


def test_online_business_knowledge_reuses_matching_brand_topic_but_rejects_replacement_topic() -> None:
    brand_rows = [
        {
            "产品类型": "笔记本",
            "核心问题": "笔记本硬盘品牌是否为第三方",
    ...
    query = dict(brand_rows[0])
    item = _online_item("笔记本硬盘是否为品牌件")
    match = workflow_module._select_online_business_match(
        brand_rows, query, [item]
    )
    assert match is not None
    assert match[0] == "auto_reuse"
    assert match[1] == "BIZ-TEST-001"

    replacement = _online_item("笔记本硬盘是否为第三方更换部件")
    replacement_match = workflow_module._select_online_business_match(
        brand_rows, query, [replacement]
    )
    assert replacement_match is None


def test_online_business_knowledge_keeps_find_outcomes_separate_and_routes_ambiguous_to_review() -> None:
    rows = [
        {
            "产品类型": "耳机/耳麦",
            "核心问题": "耳机查找功能异常是否影响回收",
    ...
    query = dict(rows[0])
    recoverable = _online_item(
        "耳机查找功能异常是否影响回收",
        product_type="耳机/耳麦",
    )
    capability = _online_item(
        "耳机是否支持定位查找功能",
        product_type="耳机/耳麦",
    )
    match = workflow_module._select_online_business_match(
        rows, query, [recoverable]
    )
    assert match is not None and match[0] == "auto_reuse"
    assert workflow_module._select_online_business_match(
        rows, query, [capability]
    ) is None
    ambiguous = workflow_module._select_online_business_match(
        rows,
        query,
        [
            recoverable,
    ...
    assert ambiguous is not None and ambiguous[0] == "manual_review"


def test_online_business_boundary_unknown_never_auto_reuses_generic_candidate() -> None:
    rows = [
        {
            "产品类型": "笔记本",
            "核心问题": "笔记本硬盘品牌是否为第三方",
    ...
    generic = _online_item("笔记本硬盘如何处理")
    match = workflow_module._select_online_business_match(rows, dict(rows[0]), [generic])
    assert match is not None
    assert match[0] == "manual_review"
    assert match[5] == "boundary_unknown"
    assert "storage_brand" in match[6]


def test_online_business_structured_boundary_projection_can_resolve_generic_title() -> None:
    rows = [
        {
            "产品类型": "笔记本",
            "核心问题": "笔记本硬盘品牌是否为第三方",
    ...
    item = _online_item(
        ...
        boundary_projection={
            "object_key": "硬盘品牌",
            "query_target": "硬盘品牌属性",
            "conclusion_state": "硬盘是否为品牌件",
        },
    )
    match = workflow_module._select_online_business_match(rows, dict(rows[0]), [item])
    assert match is not None
    assert match[0] == "auto_reuse"
    assert match[5] == "boundary_complete"
    assert "object_key=硬盘品牌" in match[6]


def test_failed_topic_transcription_keeps_source_backed_recommended_reply() -> None:
    rows = [
        {
            "工单ID": "local-test-card-slot-waterproof",
            "聊天内容": "卡槽防水标颜色发生变化，如何核验？",
            "产品类型": "手机",
            "核心问题": "卡槽防水标变化如何核验",
    ...
    topic = _failed_topic_transcription_row(
        "TOP-LOCAL-TEST",
        ("手机", "检测核验", "卡槽防水标", "防水标颜色变化"),
        rows,
        {"knowledge_value": "待确认", "confidence": 0.7},
        provider="mimo",
        model_name="mimo-v2.5",
        prompt_version="test",
        model_run_id="run-local-test",
    ...
    assert topic["推荐回复"]
    assert "防水标" in topic["推荐回复"]
    assert topic["流程状态"] == "review_pending"
...
def test_cluster_unit_payload_compacts_timestamp_and_system_lines_without_mutating_source() -> None:
    original_chat = (
        "26/08/09 20:51:44:44:44 工程师：屏幕漏液最终按折痕处理\n"
        "26/08/09 20:51:52:52:52 预览\n"
        "[图片 × 1]\n"
        "26/08/09 20:52:07:07:07 客服：另外确认硬盘是否为品牌件\n"
        "26/08/09 20:52:53:53:53 已加载全部"
    )
    source_row = {
        "工单ID": "COMPACT-001",
        "产品类型": "笔记本",
    ...
    payload = mimo_module._cluster_unit_input_payload(
        source_row,
        media_policy="never",
    )
    ...
    assert payload["intent_evidence"] == payload["primary_conversation_evidence"]
    assert payload["unrelated_followup_evidence"] == ""
    assert source_row["聊天内容"] == original_chat
...
def test_topic_signal_payload_demotes_exploratory_first_turn() -> None:
    payload = _topic_signal_source_payload(
        {
            "工单ID": "INTENT-FIRST-TURN-001",
            "产品类型": "手机",
            "聊天内容": (
                "问题类型：质检问题 问题描述：是不是漏液？ 转人工原因：先快速确认\n"
                "26/09/03 10:00:01 工程师：看完图片后确认是折痕，按外观问题判定\n"
                "26/09/03 10:00:20 答疑：不是漏液，按折痕标准处理"
            ),
            "历史实际回复": "不是漏液，按折痕标准处理。",
            "核心问题": "确认屏幕异常属于折痕还是漏液",
            "判定结论": "人工确认是折痕，不是漏液",
    ...
    assert evidence["conversation"] == (
        "工程师：看完图片后确认是折痕，按外观问题判定\n"
        "答疑：不是漏液，按折痕标准处理"
    )
    assert evidence["first_turn_candidate"] == ""
    assert evidence["intent_evidence"] == evidence["conversation"]


def test_topic_signal_payload_marks_real_first_turn_as_unconfirmed_candidate() -> None:
    payload = _topic_signal_source_payload(
        {
            "工单ID": "INTENT-FIRST-TURN-002",
            "产品类型": "手机",
            "聊天内容": "26/09/03 10:00:01 工程师：屏幕色斑怎么判？",
        }
    )

    evidence = payload["primary_evidence"]
    assert evidence["first_turn_candidate"] == "工程师：屏幕色斑怎么判？"
    assert evidence["first_turn_role"] == "exploratory_unconfirmed"
    assert evidence["intent_evidence"] == ""


def test_topic_signal_payload_splits_same_line_timestamped_turns() -> None:
    payload = _topic_signal_source_payload(
        {
            "工单ID": "INTENT-FIRST-TURN-003",
            "产品类型": "手机",
            "聊天内容": (
                "26/09/03 10:00:01 工程师：可能是漏液怎么判？ "
                "26/09/03 10:00:20 答疑：看图后确认是折痕，不是漏液。"
            ),
        }
    )

    evidence = payload["primary_evidence"]
    assert evidence["first_turn_role"] == "exploratory_demoted"
    assert evidence["first_turn_candidate"].endswith("可能是漏液怎么判？")
    assert evidence["intent_evidence"].endswith("看图后确认是折痕，不是漏液。")


def test_topic_signal_payload_uses_first_engineer_turn_not_customer_probe() -> None:
    payload = _topic_signal_source_payload(
        {
            "工单ID": "INTENT-FIRST-TURN-004",
            "产品类型": "手机",
            "聊天内容": (
                "26/09/03 10:00:00 客户：是不是漏液？\n"
                "26/09/03 10:00:08 工程师：看图确认是折痕，按外观标准处理"
            ),
        }
    )

    evidence = payload["primary_evidence"]
    assert evidence["first_turn_speaker"] == "工程师"
    assert evidence["first_turn_candidate"] == ""
    assert evidence["first_turn_role"] == "not_flagged"
    assert "看图确认是折痕" in evidence["intent_evidence"]


def test_topic_signal_payload_does_not_promote_customer_only_turn() -> None:
    payload = _topic_signal_source_payload(
        {
            "工单ID": "INTENT-FIRST-TURN-005",
            "产品类型": "手机",
            "聊天内容": "26/09/03 10:00:00 客户：是不是漏液？",
        }
    )
    evidence = payload["primary_evidence"]
    assert evidence["first_turn_role"] == "unknown_speaker"
    assert evidence["first_turn_candidate"] == ""
    assert evidence["intent_evidence"] == ""


def test_roleless_exploratory_turn_prefers_confirmation_and_isolates_new_followup() -> None:
    payload = _topic_signal_source_payload(
        {
            "工单ID": "INTENT-FIRST-TURN-006",
            "产品类型": "手机",
            "聊天内容": (
                "可能是漏液怎么判？\n"
                "看图后确认是折痕，不是漏液。\n"
    ...
    evidence = payload["primary_evidence"]
    assert evidence["first_turn_role"] == "exploratory_demoted"
    assert evidence["intent_evidence"] == "看图后确认是折痕，不是漏液。"
    assert evidence["unrelated_followup_evidence"] == "另外蓝牙也连不上，怎么处理？"
...
def test_direct_mimo_marks_unconfirmed_exploratory_first_turn_for_review() -> None:
    class ExploratoryFirstTurnMimo:
        config = SimpleNamespace(model="mimo-first-turn-review-test")
                    ...
                    "topics": [
                        {
                            ...
                            "subject": "屏幕",
                            "phenomenon": "待确认",
                            "judgment_target": "确认屏幕异常类型",
                            "resolution_mode": "结合后续答疑或图片确认",
                            "standard_path": "待确认",
                            "threshold_or_exception": "无明确阈值",
                            "evidence_summary": "仅有首轮试探问题。",
                            "confidence": 0.95,
                            "requires_review": False,
                        }
                    ],
    ...
    groups, meta = _direct_mimo_topic_groups(
        [
            {
                "数据ID": "FIRST-TURN-DIRECT-001",
                "工单ID": "FIRST-TURN-DIRECT-001",
                "产品类型": "手机",
                "聊天内容": "26/09/03 10:00:01 工程师：可能是漏液怎么判？",
            }
        ],
        ExploratoryFirstTurnMimo(),
    )

    assert len(groups) == 1
    row = groups[0][1][0]
    assert row["_原子首轮意图状态"] == "exploratory_unconfirmed"
    assert row["_原子需要复核"] is True
    assert "首轮会话仅为试探性提问" in row["人工优先复核原因"]
    assert meta["atomic_exploratory_first_turn_unconfirmed"] == 1
...
def test_atomic_completeness_protection_flags_merged_independent_conclusions() -> None:
    source_row = {
        "产品类型": "平板电脑",
        "核心问题": (
            "屏幕工具读出异常是否要看闪光图；"
    ...
    model_topics = [
        {
            "normalized_issue": (
                "平板电脑｜屏幕功能、重启记录｜"
                "工具读出异常、重启记录iOS｜按工具结果处理"
            ),
            "subject": "屏幕功能、重启记录",
            "phenomenon": "工具读出异常、重启记录iOS",
            "judgment_target": "按工具结果处理",
            "resolution_mode": "按工具结果处理",
            "evidence_summary": "模型把两个结论合并。",
            "requires_review": False,
        }
    ]

    protected, reason = workflow_module._apply_atomic_topic_completeness_protection(
        source_row,
        model_topics,
    )

    assert "未保持一项一原子问题拆分" in reason
    assert "屏幕验机工具结果" in reason
    assert "有重启记录iOS" in reason
    assert protected[0]["requires_review"] is True
    assert protected[0]["_atomic_completeness_review_reason"] == reason
    assert model_topics[0]["requires_review"] is False


def test_atomic_completeness_protection_flags_uncovered_independent_conclusion() -> None:
    source_row = {
        "产品类型": "平板电脑",
        "核心问题": "屏幕工具读出异常；判定有重启记录iOS是否需要勾选",
        "判定结论": "屏幕工具读数正常；有重启记录iOS应单独判定。",
    }
    model_topics = [
        {
            "normalized_issue": "平板电脑｜屏幕｜验机工具读出异常｜按工具结果处理",
            "subject": "屏幕",
            "phenomenon": "验机工具读出异常",
            "judgment_target": "按工具结果处理",
            "resolution_mode": "按工具结果处理",
            "evidence_summary": "只返回了屏幕工具结果。",
            "requires_review": False,
        }
    ]
    ...
    assert reason == "结构化事实包含未被原子问题覆盖的独立结论：有重启记录iOS"
    assert protected[0]["requires_review"] is True
    assert "有重启记录iOS" in protected[0]["evidence_summary"]


def test_atomic_completeness_protection_accepts_all_distinct_source_claims() -> None:
    source_row = {
        "产品类型": "笔记本",
        "核心问题": "内存和硬盘是不是品牌，转转贴纸可以不撕吗",
        "判定结论": "内存和硬盘分别核对品牌；转转贴纸不撕无影响。",
    }
    topics = [
        {
            ...
            "resolution_mode": "核对品牌信息",
            "evidence_summary": "内存品牌问题。",
            "requires_review": False,
        },
        {
            ...
            "resolution_mode": "核对品牌信息",
            "evidence_summary": "硬盘品牌问题。",
            "requires_review": False,
        },
        {
            "normalized_issue": "笔记本｜转转贴纸｜是否可撕除｜确认不撕是否影响回收",
            "subject": "转转贴纸",
            "phenomenon": "是否可撕除",
            "judgment_target": "确认不撕是否影响回收",
            "resolution_mode": "贴纸不撕不影响回收",
            "evidence_summary": "转转贴纸处理问题。",
            "requires_review": False,
        },
    ]
    ...
    assert reason == ""
    assert [topic["requires_review"] for topic in protected] == [False, False, False]
    assert all("_atomic_completeness_review_reason" not in topic for topic in protected)


def test_direct_mimo_routes_incomplete_atomic_extraction_to_priority_review() -> None:
    class CombinedAtomicMimo:
        config = SimpleNamespace(model="mimo-atomic-completeness-test")

        def analyze_cluster_units(self, _row):
            return MimoLabelResult(
                candidate={
                    "conversation_type": "single_topic",
                    "reason": "模型把两个结论合并成一个主题。",
                    "topics": [
                        {
                            "normalized_issue": (
                                "平板电脑｜屏幕功能、重启记录｜"
            ...
            return MimoLabelResult(
                candidate={
                    "clusters": [
                        {
                            "cluster_id": "C001",
                            "theme_name": "平板电脑检测结果如何判定",
                            "member_atomic_ids": [unit["unit_id"] for unit in units],
                            "merge_basis": "模型将当前原子问题保留为单成员主题。",
                        }
                    ],
                    "split_requests": [],
                    "review_requests": [],
    ...
    groups, meta = _direct_mimo_topic_groups(
        [
            {
                "数据ID": "ATOMIC-COMPLETE-001",
                "工单ID": "ATOMIC-COMPLETE-001",
                "产品类型": "平板电脑",
                "核心问题": (
                    "屏幕工具读出异常是否要看闪光图；"
                    "同时判定有重启记录iOS是否需要勾选"
    ...
    row = groups[0][1][0]
    assert row["_原子需要复核"] is True
    assert "原子输出未保持一项一原子问题拆分" in row["人工优先复核原因"]
    assert meta["atomic_completeness_review"] == 1
    assert meta["atomic_completeness_review_samples"] == [
        {
            "sample_id": "ATOMIC-COMPLETE-001",
            "reason": (
                "结构化事实包含多个独立结论，但原子输出未保持"
                "一项一原子问题拆分：屏幕验机工具结果、有重启记录iOS"
            ),
        }
    ]
...
def test_unworthy_topic_is_classified_before_transcription_and_skips_draft_generation() -> None:
    ...
    reply = topics[0]["推荐回复"]
    assert reply
    assert "外壳位置是否存在可见磕碰" in reply
    assert "补拍全景和近景" in reply
    assert "不可仅凭单张图片确定" in reply
    assert "当前单个案例结论" not in reply
...
def test_untranscribed_topic_title_uses_issue_description_not_chat_header() -> None:
    ...
    assert "手机" in topics[0]["主标题"]
    assert "判定" in topics[0]["主标题"]
        ...
        def label_topic(self, _topic, _matches, use_standard_references=False):
            ...
            return MimoLabelResult(
                candidate={
                    ...
                    "standard_refs": [],
                    "applicable_scope": "手机-通用",
                    "recommended_reply": "请先进入设备信息页读取配置，再使用检测工具交叉核对；设备页面和检测工具需要依次交叉核对。",
                    "confidence": 0.9,
...
def test_topic_quality_review_does_not_reclassify_knowledge_value() -> None:
    ...
    assert "knowledge_value 仅作兼容字段" in prompt
...
def test_direct_mimo_repairs_duplicate_atomic_ids_before_clustering() -> None:
    ...
    rows = [
        ...
        for conversation in ("屏幕色斑怎么判", "屏幕色斑如何判定")
    ]
    clustering_meta: dict[str, object] = {}
...
def test_direct_mimo_caches_successful_timeout_split_retry_batches(
    tmp_path: Path,
) -> None:
    ...
    assert first_meta["direct_cluster_failure_category_counts"] == {
        "timeout": 1
    }
...
def test_direct_mimo_locally_splits_notebook_model_and_hardware_brand_queries() -> None:
    ...
    assert len(groups) == 3
    assert targets == {"model_query", "memory_brand", "storage_brand"}
    assert meta["local_multi_topic_rescue"] == 1


def test_direct_mimo_locally_splits_notebook_model_and_region_version_queries() -> None:
    class CombinedNotebookVersionMimo:
        config = SimpleNamespace(model="mimo-local-version-split-test")
                    ...
                    "topics": [
                        {
                            ...
                            "standard_path": "设备信息查询",
                            "threshold_or_exception": "无明确阈值",
                            "evidence_summary": "聊天中分别确认型号与国行零售版本。",
                            "confidence": 0.9,
                            "requires_review": False,
                        }
                    ]
    ...
    groups, meta = _direct_mimo_topic_groups(
        [
            {
                "数据ID": "NOTEBOOK-MODEL-VERSION-001",
                "工单ID": "NOTEBOOK-MODEL-VERSION-001",
                "产品类型": "笔记本",
                "聊天内容": "请确认设备型号；再确认该版本是否为国行零售机。",
            }
        ],
        CombinedNotebookVersionMimo(),
        batch_size=4,
    )
    ...
    assert len(groups) == 2
    assert targets == {"model_query", "device_version"}
    assert meta["local_multi_topic_rescue"] == 1


def test_cluster_only_device_version_title_excludes_model_query_wording() -> None:
    row = {
        "数据ID": "NOTEBOOK-REGION-VERSION-001",
        "工单ID": "NOTEBOOK-REGION-VERSION-001",
        "产品类型": "笔记本",
        "回收业务层级": "自营回收",
        "对象/部位": "设备型号/版本",
        "异常现象": "国行零售机身份确认",
        "判定目标": "确认设备版本属性",
        "解题方式": "根据型号后缀确认国行零售机身份",
        "核心问题": "笔记本设备型号和版本确认是否为国行零售机",
        "问题意图": "信息查询",
        "_聚类主题标题": "笔记本设备型号和版本国行零售机身份如何确认",
        "聊天内容": "这是国行的吗？这是零售机吗？",
    }

    topic = workflow_module._cluster_only_topic_row(
        "TOP-NOTEBOOK-REGION-VERSION",
        ("direct_mimo", "笔记本"),
        [row],
    )

    assert topic["聚类主题"] == "笔记本国行零售机版本如何确认"
    assert "型号和版本" not in topic["聚类主题"]


def test_cluster_only_country_context_does_not_relabel_reset_process_as_version() -> None:
    row = {
        "数据ID": "GAME-CONSOLE-RESET-001",
        "工单ID": "GAME-CONSOLE-RESET-001",
        "产品类型": "游戏机",
        "回收业务层级": "自营回收",
        "对象/部位": "系统初始化设置",
        "异常现象": "设备能连Wi-Fi但无网络，无法完成联网验证进行恢复出厂设置",
        "判定目标": "完成恢复出厂设置操作",
        "解题方式": "通过无法连接互联网选项跳过联网验证",
        "核心问题": "国行任天堂Switch无法联网验证时如何恢复出厂设置",
        "问题意图": "流程操作",
        "聊天内容": "这台国行Switch无法联网验证，怎么恢复出厂设置？",
    }

    topic = workflow_module._cluster_only_topic_row(
        "TOP-GAME-CONSOLE-RESET",
        ("direct_mimo", "游戏机"),
        [row],
    )

    assert "恢复出厂设置" in topic["聚类主题"]
    assert "国行零售机版本" not in topic["聚类主题"]


def test_direct_mimo_locally_splits_notebook_brand_and_missing_screw_queries() -> None:
    class BrandAndScrewMimo:
        config = SimpleNamespace(model="mimo-local-brand-screw-split-test")
        ...
        def analyze_cluster_units(self, _row):
            return MimoLabelResult(
                candidate={
                    "topics": [
                        {
                            "normalized_issue": "笔记本内部螺丝缺失如何判定",
                            "product_category": "笔记本",
                            "scope_type": "品类专用",
                            "platform": "通用",
                            "brand": "通用",
                            ...
                            "subject": "内部螺丝",
                            "phenomenon": "螺丝缺失",
                            "judgment_target": "确认是否存在非官方拆卸痕迹",
                            "resolution_mode": "按拆修标准判定",
                            "standard_path": "内部螺丝缺失",
                            "threshold_or_exception": "无明确阈值",
                            "evidence_summary": "模型只抽出了螺丝缺失问题。",
                            "confidence": 0.9,
                            "requires_review": False,
                        }
                    ]
    ...
    groups, meta = _direct_mimo_topic_groups(
        [
            {
                "数据ID": "NOTEBOOK-BRAND-SCREW-001",
                "工单ID": "NOTEBOOK-BRAND-SCREW-001",
                "产品类型": "笔记本",
                "聊天内容": (
                    "问题描述：内存和硬盘是不是品牌。"
                    "拆机图里好像没有螺丝。内部螺丝缺失。"
    ...
    assert len(groups) == 3
    assert targets == {"memory_brand", "storage_brand", ""}
    assert meta["local_multi_topic_rescue"] == 1


def test_local_notebook_brand_component_split_preserves_existing_screw_topic() -> None:
    source = {
        "产品类型": "笔记本",
        "聊天内容": "内存硬盘帮看一下，内部螺丝缺失需要怎么判？",
    }
    combined_brand = {
        "normalized_issue": "笔记本内存和硬盘是否为品牌件",
        "subject": "内存和硬盘",
        "phenomenon": "是否为品牌件",
    ...
    screw = {
        ...
        "subject": "内部螺丝",
        "phenomenon": "螺丝缺失",
        "judgment_target": "确认是否存在拆修痕迹",
        "confidence": 0.9,
        "requires_review": False,
    }

    split, applied = workflow_module._direct_local_split_notebook_brand_topics(
        source,
        [combined_brand, screw],
    )

    assert applied is True
    assert {topic["subject"] for topic in split} == {"内存", "硬盘", "内部螺丝"}
    assert all(
        topic["subject"] != "内存和硬盘"
        for topic in split
    )
...
def test_direct_reconcile_merges_phone_leakage_topics_with_explanatory_text() -> None:
    def row(record_id: str, phenomenon: str, issue: str) -> dict[str, object]:
        return {
            "数据ID": record_id,
            "工单ID": record_id,
            "_原子知识ID": f"{record_id}-U1",
            "产品类型": "手机",
            "模型主题一级分类": "显示问题",
            "模型主题二级分类": "屏幕显示异常",
            "问题意图": "标准判定",
            ...
            "异常现象": phenomenon,
            "核心问题": issue,
            "判定目标": "判断手机屏幕漏液是否符合回收标准",
            "解题方式": "按屏幕显示异常标准判定",
            "主标准路径": "屏幕显示情况-屏幕显示异常-屏幕有漏液/条纹/闪烁/图层错乱",
            "_原子平台": "通用",
            "_原子品牌": "通用",
            "_原子机型范围": "通用",
    ...
    groups = [
        (("direct_mimo", "手机", "first"), [
            row("PHONE-LEAK-A", "漏液（显示绿色、粉色色块）", "手机｜屏幕｜漏液（显示绿色、粉色色块）｜判定是否符合回收标准"),
        ]),
        (("direct_mimo", "手机", "second"), [
            row("PHONE-LEAK-B", "漏液", "手机｜屏幕｜漏液｜判定屏幕显示异常及回收条件"),
        ]),
    ]
    meta: dict[str, object] = {}
    reviewer = SimpleNamespace(config=SimpleNamespace(model="offline-leakage-test"))

    reconciled = workflow_module._reconcile_direct_topic_groups(groups, reviewer, meta)

    assert len(reconciled) == 1
    assert len(reconciled[0][1]) == 2
    assert meta["direct_reconcile_rule_approved"] == 1


def test_direct_reconcile_merges_camera_lens_mark_aliases_across_batches() -> None:
    def row(record_id: str, subject: str, phenomenon: str) -> dict[str, object]:
        return {
            "数据ID": record_id,
            "工单ID": record_id,
            "_原子知识ID": f"{record_id}-U1",
            ...
            "对象/部位": subject,
            "异常现象": phenomenon,
            "核心问题": f"手机{subject}{phenomenon}如何判定",
            "判定目标": "判断是否需要勾选镜头印记",
            "解题方式": "按标准选择对应项",
            "主标准路径": "镜头-印记/擦洗/异物/脏污/指印",
            "_原子平台": "通用",
            "_原子品牌": "通用",
            "_原子机型范围": "通用",
            "_聚类裁决提供方": "mimo-direct",
            "_聚类主题标题": "手机疑似拆修或维修痕迹如何核验",
        }

    first = row("CAMERA-MARK-A", "后置摄像头镜头", "镜头表面存在印记/污迹")
    second = row("CAMERA-MARK-B", "摄像头镜头", "镜头内部存在异物、印记或污迹")
    meta: dict[str, object] = {}
    reviewer = SimpleNamespace(config=SimpleNamespace(model="offline-camera-mark-test"))

    assert not workflow_module._direct_reconcile_has_hard_conflict(first, [second])
    reconciled = workflow_module._reconcile_direct_topic_groups(
        [
            (("direct_mimo", "手机", "first"), [first]),
            (("direct_mimo", "手机", "second"), [second]),
        ],
        reviewer,
        meta,
    )
...
def test_direct_reconcile_merges_camera_lens_marks_after_conflict_split() -> None:
    def row(
        record_id: str,
        subject: str,
        phenomenon: str,
    ...
    rear_camera = row(
        ...
        "后置摄像头镜头",
        "镜头表面存在印记或污渍",
        "mimo-direct-reconcile-rule",
    )
    meta: dict[str, object] = {}
    reviewer = SimpleNamespace(config=SimpleNamespace(model="offline-camera-post-guard-test"))

    reconciled = workflow_module._reconcile_direct_topic_groups(
        [
            (("direct_mimo", "手机", "post-guard"), [post_guard_camera]),
            (("direct_mimo", "手机", "rear"), [rear_camera]),
        ],
        reviewer,
        meta,
    )
...
def test_direct_reconcile_merges_notebook_third_party_storage_queries() -> None:
    def row(
        record_id: str,
        *,
        subject: str,
        ...
        return {
            ...
            "模型主题二级分类": category_l2,
            "问题意图": "信息查询",
            "对象/部位": subject,
            "异常现象": phenomenon,
            "核心问题": issue,
            "判定目标": judgment_target,
            "解题方式": resolution_mode,
            "主标准路径": "通过设备信息和现场证据确认部件来源",
            "_原子平台": "通用",
            "_原子品牌": "通用",
            "_原子机型范围": "通用",
            "_原子阈值例外": "无明确阈值",
            "_聚类裁决提供方": "mimo-direct",
            "_聚类主题标题": "笔记本内存和硬盘是否为第三方更换部件如何判定",
        }

    memory_and_storage = row(
        "NOTEBOOK-THIRD-PARTY-A",
        subject="内存和硬盘",
        phenomenon="是否为第三方更换部件",
        issue="笔记本内存和硬盘是否为第三方更换部件",
        judgment_target="确认内存和硬盘是否为第三方部件",
        resolution_mode="依据系统信息和现场证据确认部件来源",
        category_l1="信息查询",
        category_l2="硬件配置查询",
    )
    storage_only = row(
        "NOTEBOOK-THIRD-PARTY-B",
        subject="硬盘",
        phenomenon="第三方硬盘判定",
        issue="笔记本硬盘第三方硬盘如何判定",
        judgment_target="确认硬盘是否为第三方更换部件",
        resolution_mode="依据系统信息和现场证据确认部件来源",
        category_l1="拆修问题",
        category_l2="硬盘更换/加装/第三方硬盘",
    )
    meta: dict[str, object] = {}
    reviewer = SimpleNamespace(config=SimpleNamespace(model="offline-third-party-test"))

    reconciled = workflow_module._reconcile_direct_topic_groups(
        [
            (("direct_mimo", "笔记本", "first"), [memory_and_storage]),
            (("direct_mimo", "笔记本", "second"), [storage_only]),
        ],
        reviewer,
        meta,
    )
...
def test_direct_reconcile_merges_notebook_memory_brand_queries() -> None:
    def row(
        record_id: str,
        phenomenon: str,
        issue: str,
        ...
        return {
            ...
            "_原子阈值例外": "无明确阈值",
            "_聚类裁决提供方": "mimo-direct",
            "_聚类主题标题": "笔记本内存是否为品牌件如何确认",
        }

    cxmt = row(
        "NOTEBOOK-MEMORY-BRAND-A",
        "内存品牌为CXMT（长鑫存储），回收师表示没见过",
        "笔记本内存品牌为CXMT如何判定",
        "确认内存品牌是否正规",
    )
    brand_part = row(
        "NOTEBOOK-MEMORY-BRAND-B",
        "是否为品牌件",
        "笔记本内存是否为品牌件",
        "确认内存品牌属性",
    )
    meta: dict[str, object] = {}
    reviewer = SimpleNamespace(config=SimpleNamespace(model="offline-memory-brand-test"))

    reconciled = workflow_module._reconcile_direct_topic_groups(
        [
            (("direct_mimo", "笔记本", "first"), [cxmt]),
            (("direct_mimo", "笔记本", "second"), [brand_part]),
        ],
        reviewer,
        meta,
    )
...
def test_direct_reconcile_merges_notebook_storage_brand_third_party_wording() -> None:
    def row(record_id: str, phenomenon: str, issue: str) -> dict[str, object]:
        return {
            "数据ID": record_id,
            "工单ID": record_id,
    ...
    third_party_brand = row(
        "NOTEBOOK-STORAGE-BRAND-A",
        "品牌是否为第三方",
        "笔记本硬盘品牌是否为第三方",
    )
    brand_part = row(
        "NOTEBOOK-STORAGE-BRAND-B",
        "是否为品牌件",
        "笔记本硬盘是否为品牌件",
    )
    evidence_based_third_party = {
        **row(
            "NOTEBOOK-STORAGE-BRAND-C",
            "第三方硬盘判定",
            "笔记本硬盘第三方硬盘如何判定",
        ),
        "模型主题一级分类": "拆修问题",
        "模型主题二级分类": "硬盘更换/加装/第三方硬盘",
        "判定目标": "是否判定为第三方硬盘",
        "解题方式": "依据图片证据和判定规则进行判定",
        "主标准路径": "需有明确证据证明其为非原厂品牌或后更换部件",
        "聊天内容": "硬盘信息没有显示可识别品牌，是否要判第三方硬盘？不判。",
        "机型": "华为 擎云 S540",
        "语义标注依据": "图片仅显示硬盘容量信息，未显示品牌型号。",
    }
    meta: dict[str, object] = {}
    reviewer = SimpleNamespace(config=SimpleNamespace(model="offline-storage-brand-test"))

    reconciled = workflow_module._reconcile_direct_topic_groups(
        [
            (("direct_mimo", "笔记本", "first"), [third_party_brand]),
            (("direct_mimo", "笔记本", "second"), [brand_part]),
            (("direct_mimo", "笔记本", "third"), [evidence_based_third_party]),
        ],
        reviewer,
        meta,
    )
    ...
    assert len(reconciled) == 1
    assert len(reconciled[0][1]) == 3
    assert {
        workflow_module._direct_reconcile_fingerprint(item).query_target
        for item in reconciled[0][1]
    } == {"storage_brand"}
    assert meta["direct_reconcile_rule_approved"] == 2
    topic = workflow_module._cluster_only_topic_row(
        "TOP-STORAGE-BRAND",
        reconciled[0][0],
        reconciled[0][1],
    )
    assert topic["聚类主题"] == "笔记本硬盘是否为品牌件"


def test_topic_rule_draft_rebuilds_generic_repair_title_from_structured_topic() -> None:
    camera_rows = [
        {
            "数据ID": "CAMERA-TITLE",
            "工单ID": "CAMERA-TITLE",
            ...
            "对象/部位": "后置摄像头镜头",
            "异常现象": "镜头表面存在印记或污渍",
            "核心问题": "手机疑似拆修或维修痕迹如何核验",
            "判定目标": "判断是否需要勾选镜头印记",
            "解题方式": "按标准选择对应项",
            "聊天内容": "后置摄像头镜头有印记，需要按镜头印记判定。",
            "历史实际回复": "按镜头-印记/擦洗/异物/脏污/指印处理。",
        }
    ]
    notebook_rows = [
        {
            ...
            "对象/部位": "笔记本屏幕",
            "异常现象": "屏幕边缘缝隙",
            "核心问题": "笔记本疑似拆修或维修痕迹如何核验",
            "判定目标": "判断是否为屏幕脱胶",
            "解题方式": "使用1mm塞规检测屏幕与边框间隙",
            "聊天内容": "使用1mm塞规检测屏幕与边框间隙。",
            "历史实际回复": "塞规能够插入则判定为屏幕脱胶。",
        }
    ]

    camera_draft = workflow_module._topic_rule_draft(
        "TOP-CAMERA-TITLE",
        camera_rows,
        [],
        use_standard_references=False,
    )
    notebook_draft = workflow_module._topic_rule_draft(
        "TOP-NOTEBOOK-GAP",
        notebook_rows,
        [],
        use_standard_references=False,
    ...
    assert "疑似拆修或维修痕迹" not in camera_draft["title"]
    assert "后置摄像头镜头" in camera_draft["title"]
    assert "镜头镜头" not in camera_draft["title"]
    assert "疑似拆修或维修痕迹" not in notebook_draft["title"]
    assert "笔记本屏幕" in notebook_draft["title"]
    assert camera_draft["title"] != notebook_draft["title"]


def test_cluster_only_title_rejects_table_band_title_for_charging_line_topic() -> None:
    row = {
        "数据ID": "WATCH-CHARGING-LINE",
        "工单ID": "WATCH-CHARGING-LINE",
        "_原子知识ID": "WATCH-CHARGING-LINE-U2",
        "产品类型": "智能手表",
        "模型主题一级分类": "配件问题",
        "模型主题二级分类": "配件缺失",
        "问题意图": "完整性确认",
        ...
        "判定目标": "确认配件是否齐全",
        "解题方式": "记录配件缺失事实",
        "核心问题": "智能手表｜充电配件｜缺少充电线（仅有充电底座）｜配件完整性确认",
        "原始核心问题": "同时咨询更换表带判定和充电线缺失。",
        "判定结论": "充电线缺失属于配件缺失。",
        "聊天内容": "只有充电底座，充电线没有了。",
        "_聚类主题标题": "手表更换表带如何判定",
    }
    ...
    topic = workflow_module._cluster_only_topic_row(
        "TOP-WATCH-CHARGING-LINE",
        ("direct_mimo", "智能手表"),
        [row],
    )

    assert workflow_module._topic_has_multiple_targets(
        {
            "核心问题": row["核心问题"],
            "人工核心问题": row["原始核心问题"],
            "人工判定结论": row["判定结论"],
    ...
    assert "充电线" in topic["聚类主题"]
    assert "表带" not in topic["聚类主题"]


def test_cluster_title_requires_business_object_and_phenomenon() -> None:
    query = {
        "对象/部位": "后盖",
        "异常现象": "碎裂痕迹",
        "判定目标": "判定后盖是否属于外壳碎裂",
    ...
    assert not workflow_module._cluster_title_matches_topic(
        "后盖素皮如何判定", query
    )
    assert not workflow_module._cluster_title_matches_topic(
        "无帮助如何判定", {
    ...
    assert workflow_module._cluster_title_matches_topic(
        "手机外观碎裂判定",
        {"对象/部位": "外观", "异常现象": "疑似碎裂", "判定目标": "判断外观是否碎裂"},
    )


def test_structured_cluster_title_uses_business_fields_not_user_description() -> None:
    cases = (
        ("看一下后摄如何判定", "后置摄像头镜头", "镜头表面存在印记/污迹", ("后置摄像头镜头", "印记", "污迹")),
        ("后盖素皮如何判定", "后盖", "碎裂痕迹", ("后盖", "碎裂")),
        ("无帮助如何判定", "屏幕外玻璃", "磕点/划痕", ("屏幕外玻璃", "磕点", "划痕")),
    )
    for raw_title, subject, phenomenon, expected in cases:
        row = {
            "产品类型": "手机",
            "回收业务层级": "自营回收",
            "对象/部位": subject,
            ...
            "解题方式": "按业务标准判定",
            "_聚类主题标题": raw_title,
            "聊天内容": f"问题描述：{raw_title}",
        }
        title = workflow_module._cluster_only_topic_row(
            "TOP-TITLE-BUSINESS",
            ("direct_mimo", "手机"),
            [row],
        )["聚类主题"]
        assert all(term in title for term in expected)
        assert title != raw_title
        assert "镜头镜头" not in title


def test_cluster_only_real_titles_remove_repeated_object_words() -> None:
    cases = (
        (
            {
                "产品类型": "笔记本",
        ...
        (
            {
                "产品类型": "笔记本",
                "对象/部位": "转轴/合盖",
                "异常现象": "合盖后存在明显缝隙，无法完全闭合",
                "核心问题": (
                    "笔记本｜转轴/合盖｜合盖后存在明显缝隙，无法完全闭合｜"
                    "判定为转轴功能异常"
                ),
            },
            "笔记本合盖后存在明显缝隙且无法完全闭合如何判定",
        ),
        (
            {
                "产品类型": "手写笔",
                ...
                "异常现象": "“查找”功能是否已退出",
                "核心问题": (
                    "手写笔｜Apple Pencil Pro｜“查找”功能是否已退出｜"
                    "判定设备状态是否符合回收标准"
                ),
                "原始核心问题": (
                    "回收师询问Apple Pencil Pro在iPad上的“查找”功能是否已经退出，"
                    "以确认设备状态是否符合回收标准。"
                ),
                "机型": "Apple Pencil Pro",
                "_原子机型范围": "Apple Pencil Pro",
            },
            "Apple Pencil Pro 是否已退出“查找”功能",
        ),
    )

    for index, (case, expected_title) in enumerate(cases, start=1):
        row = {
            "数据ID": f"REAL-TITLE-REPEAT-{index}",
            "工单ID": f"REAL-TITLE-REPEAT-{index}",
            "回收业务层级": "自营回收",
            "问题意图": "标准判定",
            "解题方式": "按来源业务事实判定",
            **case,
        ...
        topic = workflow_module._cluster_only_topic_row(
            f"TOP-REAL-TITLE-REPEAT-{index}",
            ("direct_mimo", row["产品类型"]),
            [row],
        )

        assert topic["聚类主题"] == expected_title


def test_cluster_only_real_title_rebuilds_recycler_question_as_business_question() -> None:
    row = {
        "数据ID": "REAL-TITLE-CHARGING-CABLE",
        "工单ID": "REAL-TITLE-CHARGING-CABLE",
        "回收业务层级": "自营回收",
        "产品类型": "笔记本",
        "对象/部位": "充电线",
        "异常现象": "回收师询问如何判断充电线是否为原装",
        "核心问题": "笔记本｜充电线｜原装性判断标准｜信息查询",
        "原始核心问题": "回收师咨询如何判断笔记本电脑充电线是否为原装。",
        "判定结论": (
            "充电线无需强制鉴定原装，只需满足适配性和物理规格要求。"
        ),
        "问题意图": "信息查询",
        "解题方式": "提供充电线适配性和物理规格检测标准",
    }

    topic = workflow_module._cluster_only_topic_row(
        "TOP-REAL-TITLE-CHARGING-CABLE",
        ("direct_mimo", "笔记本"),
        [row],
    )

    assert topic["聚类主题"] == "笔记本充电线是否需要鉴定原装"
    assert "回收师" not in topic["聚类主题"]


def test_cluster_only_real_title_keeps_huawei_matebook_third_party_boundary() -> None:
    row = {
        "数据ID": "REAL-TITLE-HUAWEI-MATEBOOK",
        "工单ID": "REAL-TITLE-HUAWEI-MATEBOOK",
        "回收业务层级": "自营回收",
        "产品类型": "笔记本",
        "对象/部位": "内存和硬盘",
        "异常现象": "回收师对内存和硬盘是否为第三方更换部件存在疑问",
        "核心问题": "笔记本｜内存和硬盘｜第三方部件判断｜信息查询",
        "原始核心问题": (
            "回收师在现场回收华为MateBook 14 2024笔记本时，"
            "对设备的内存和硬盘是否为第三方更换部件存在疑问。"
        ),
        "判定结论": "该设备内存和硬盘无需判定为第三方部件。",
        "机型": "华为 MateBook 14 2024",
        "问题意图": "信息查询",
        "解题方式": "根据配置信息和无反证原则进行判断",
    }

    topic = workflow_module._cluster_only_topic_row(
        "TOP-REAL-TITLE-HUAWEI-MATEBOOK",
        ("direct_mimo", "笔记本"),
        [row],
    )

    assert topic["聚类主题"] == "华为 MateBook 内存和硬盘是否为第三方更换部件"
    assert "回收师" not in topic["聚类主题"]


def test_cluster_only_real_title_keeps_unspecified_configuration_case_manual() -> None:
    row = {
        "数据ID": "REAL-TITLE-CONFIGURATION-MANUAL",
        "工单ID": "REAL-TITLE-CONFIGURATION-MANUAL",
        "回收业务层级": "自营回收",
        "产品类型": "笔记本",
        "对象/部位": "设备配置信息",
        "异常现象": "回收师提供设备配置信息图片，未描述具体问题",
        "核心问题": "笔记本｜设备配置信息｜信息查询｜寻求指导",
        "原始核心问题": (
            "回收师在回收现场遇到问题复杂不确定如何处理，"
            "提供了设备配置信息图片以寻求指导。"
        ),
        "判定结论": "会话未提出具体质检争议，仅能确认设备配置信息。",
        "问题意图": "其他待确认",
        "解题方式": "回收师需清晰描述具体问题点或争议点",
    }

    topic = workflow_module._cluster_only_topic_row(
        "TOP-REAL-TITLE-CONFIGURATION-MANUAL",
        ("direct_mimo", "笔记本"),
        [row],
        {
            "status": "待人工聚类复核",
    ...
    assert topic["聚类主题"] == "笔记本设备配置信息待人工确认"
    assert topic["聚类准入状态"] == "待人工聚类复核"


def test_cluster_only_remaining_second_round_titles_are_clear() -> None:
    cases = (
        (
            {
                "产品类型": "手机",
                "对象/部位": "官网信息",
                "异常现象": "查询官网信息以确认设备是否为展示机",
                "核心问题": (
                    "手机｜官网信息情况｜通过IMEI查询官网信息以确认设备是否为展示机｜"
                    "判定官网信息是否正常"
                ),
                "解题方式": "通过IMEI查询官网信息并与实物核对",
            },
            "手机展示机身份如何通过官网信息确认",
            None,
        ),
        (
            {
                ...
                "异常现象": "指南针功能异常，校正无反应",
                "核心问题": (
                    "手机｜指南针传感器｜指南针功能异常（校正无反应）｜"
                    "判定为指南针无法打开或指向异常"
                ),
                "解题方式": "按指南针传感器功能异常判定",
            },
            "手机指南针传感器功能异常且校正无反应如何判定",
            None,
        ),
        (
            {
                ...
                "对象/部位": "快门次数",
                "异常现象": "查询快门次数",
                "核心问题": "单电/微单机身｜佳能EOS R7｜查询快门次数｜信息查询",
                "原始核心问题": "该机型是否支持查询快门次数。",
                "判定结论": "佳能EOS R7不支持通过当前系统查询快门次数。",
                "解题方式": "告知查询支持情况及原因",
            },
            "佳能 EOS R7 是否支持查询快门次数",
            None,
        ),
        (
            {
                "产品类型": "笔记本",
                "对象/部位": "存储容量",
                "异常现象": "硬盘爬虫读出512GB与靓机助手显示1TB不一致",
                "核心问题": (
                    "笔记本｜存储容量｜硬盘爬虫读出512GB与靓机助手显示1TB不一致｜"
                    "判定实际容量"
                ),
                "解题方式": "确认不同检测工具读数冲突时的容量判定规则",
            },
            "笔记本硬盘容量检测结果不一致如何判定",
            None,
        ),
        (
            {
                "产品类型": "笔记本",
                "对象/部位": "屏幕与边框间隙",
                "异常现象": "屏幕边缘可能存在缝隙或脱胶迹象",
                "核心问题": (
                    "笔记本｜屏幕与边框间隙｜是否符合屏幕脱胶标准｜"
                    "使用塞规检测判定"
                ),
                "解题方式": "使用1mm厚度的塞规检测屏幕与边框之间的间隙",
            },
            "笔记本屏幕与边框间隙如何判定是否脱胶",
            None,
        ),
        (
            {
                ...
                "异常现象": "物品缺失（少一个底下的）",
                "核心问题": (
                    "耳机/耳麦｜AirPods Pro 三代｜物品缺失（少一个底下的）｜"
                    "判定是否影响回收"
                ),
                "解题方式": "需补充具体缺失物品或照片后判定",
            },
            "耳机缺失物品待人工确认",
            {
                "status": "待人工聚类复核",
    ...
    for index, (case, expected_title, admission) in enumerate(cases, start=1):
        row = {
            "数据ID": f"REAL-TITLE-REMAINING-{index}",
            "工单ID": f"REAL-TITLE-REMAINING-{index}",
            "回收业务层级": "自营回收",
            "问题意图": "标准判定",
            **case,
        }
        topic = workflow_module._cluster_only_topic_row(
            f"TOP-REAL-TITLE-REMAINING-{index}",
            ("direct_mimo", row["产品类型"]),
            [row],
            admission,
        )
...
def test_cluster_only_generic_topic_is_renamed_and_forced_to_manual_review() -> None:
    row = {
        "数据ID": "NOTEBOOK-GENERIC-001",
        "工单ID": "NOTEBOOK-GENERIC-001",
        "产品类型": "笔记本",
        "回收业务层级": "自营回收",
        "对象/部位": "待确认",
        "异常现象": "待确认",
    ...
    topic = workflow_module._cluster_only_topic_row(
        "TOP-NOTEBOOK-GENERIC",
        ("direct_mimo", "笔记本"),
        [row],
        {"status": "已自动放行", "confidence": 0.95, "reason": "test"},
    )

    assert topic["聚类主题"] == "笔记本信息不足待人工确认"
    assert topic["聚类准入状态"] == "待人工聚类复核"
    assert topic["是否重点复核"] == "是"
    assert "主题缺少可执行的对象、现象或判定目标" in topic["聚类准入原因"]


def test_cluster_only_mainboard_title_keeps_the_missing_state_boundary() -> None:
    row = {
        "数据ID": "PHONE-MAINBOARD-001",
        "工单ID": "PHONE-MAINBOARD-001",
        "产品类型": "手机",
        "回收业务层级": "自营回收",
        "对象/部位": "主板",
        "异常现象": "状态确认",
        "判定目标": "确认主板拆修情况",
        "解题方式": "需补充检测（如Xray、拆机、系统弹窗）",
        "核心问题": "手机主板状态确认，判定是否存在维修、进液等异常",
        "问题意图": "标准判定",
        "_聚类主题标题": "手机主板状态如何确认",
        "聊天内容": "请确认手机主板是否正常、维修或进液。",
    }
    ...
    assert topic["聚类主题"] == "手机主板状态（正常、维修、进液）如何判定"
    assert topic["聚类准入状态"] == "待人工聚类复核"
    assert "主题同时包含多个互斥状态" in topic["聚类准入原因"]


def test_cluster_only_source_states_do_not_genericize_non_mainboard_title() -> None:
    row = {
        "数据ID": "NOTEBOOK-TRUE-TONE-001",
        "工单ID": "NOTEBOOK-TRUE-TONE-001",
        "产品类型": "笔记本",
        "回收业务层级": "自营回收",
        "对象/部位": "屏幕",
        "异常现象": "原彩显示功能支持",
        "判定目标": "确认该机型是否具备原彩显示功能",
        "解题方式": "根据具体型号核对功能支持情况",
        "核心问题": "笔记本屏幕原彩显示功能支持如何判定",
        "人工判定结论": "该机型无原彩显示，屏幕状态正常；不属于拆修问题。",
        "问题意图": "信息查询",
        "_聚类主题标题": "笔记本屏幕原彩显示功能支持如何判定",
        "聊天内容": "请确认该机型是否支持原彩显示。",
    }

    topic = workflow_module._cluster_only_topic_row(
        "TOP-NOTEBOOK-TRUE-TONE",
        ("direct_mimo", "笔记本"),
        [row],
    )

    assert topic["聚类主题"] != "笔记本屏幕状态（正常、拆修）如何判定"
    assert "原彩显示" in topic["聚类主题"]


def test_cluster_only_generic_manual_review_titles_keep_distinct_source_objects() -> None:
    common = {
        "产品类型": "笔记本",
        "回收业务层级": "自营回收",
        "对象/部位": "待确认",
    ...
    film = workflow_module._cluster_only_topic_row(
        "TOP-NOTEBOOK-FILM-GAP",
        ("direct_mimo", "笔记本"),
        [
            {
                **common,
                "数据ID": "NOTEBOOK-FILM-GAP",
                "工单ID": "NOTEBOOK-FILM-GAP",
                "核心问题": "屏幕钢化膜是否应撕除，撕除是否会损坏屏幕",
            }
        ],
    )
    capacity = workflow_module._cluster_only_topic_row(
        "TOP-NOTEBOOK-CAPACITY-GAP",
        ("direct_mimo", "笔记本"),
        [
            {
                **common,
                "数据ID": "NOTEBOOK-CAPACITY-GAP",
                "工单ID": "NOTEBOOK-CAPACITY-GAP",
                "核心问题": "磁盘容量显示异常，实际容量如何录入",
            }
        ],
    )

    assert film["聚类主题"] == "笔记本屏幕钢化膜处理待人工确认"
    assert capacity["聚类主题"] == "笔记本磁盘容量录入待人工确认"
    assert film["聚类主题"] != capacity["聚类主题"]


def test_structured_cluster_title_disambiguates_same_raw_title_for_atomic_topics() -> None:
    raw_title = "相机电池健康如何判定"
    rows = []
    for index, (subject, phenomenon) in enumerate(
        (
            ("电池", "查询电池是否为原装正品"),
            ("电池图标", "充电时电池图标显示斜杠"),
        ),
        ...
        rows.append(
            {
                "数据ID": f"CAMERA-TITLE-{index}",
                "工单ID": f"CAMERA-TITLE-{index}",
                "产品类型": "单电/微单机身",
                "回收业务层级": "自营回收",
                "对象/部位": subject,
                "异常现象": phenomenon,
    ...
    titles = [
        workflow_module._cluster_only_topic_row(
            f"TOP-CAMERA-TITLE-{index}",
            ("direct_mimo", "自营回收", "单电/微单机身"),
            [row],
        )["聚类主题"]
        for index, row in enumerate(rows, start=1)
    ]

    assert titles[0] != raw_title
    assert titles[1] != raw_title
    assert titles[0] != titles[1]
    assert "电池是否为原装正品" in titles[0]
    assert "电池图标" in titles[1]


def test_topic_query_uses_atomic_scoped_conversation_evidence() -> None:
    row = {
        "数据ID": "ATOMIC-SCOPE-001",
        "工单ID": "ATOMIC-SCOPE-WO",
        "原始工单ID": "ATOMIC-SCOPE-WO",
        "_原子知识ID": "ATOMIC-SCOPE-WO-U1",
        "产品类型": "手机",
        "对象/部位": "屏幕",
        "异常现象": "色斑",
        "核心问题": "手机屏幕色斑如何判定",
        "聊天内容": "屏幕有色斑如何判定；另一个问题是电池健康度如何查询",
    }

    query = workflow_module._topic_query([row])

    assert "屏幕有色斑" in query["聊天内容"]
    assert "电池健康度" not in query["聊天内容"]
...
def test_direct_reconcile_merges_multi_member_clusters_for_same_trusted_target() -> None:
    ...
    assert all(len(group_rows) == 2 for _key, group_rows in reconciled)
    assert meta["direct_reconcile_rule_approved"] == 0
...
def test_cluster_only_workbook_has_one_sheet_and_skips_downstream_models(
    ...
    topic_title_cell = worksheet.cell(
        row=2,
        column=header_index["聚类主题"],
    )
    assert set(str(work_order_cell.value).splitlines()) == set(work_order_ids)
    assert work_order_cell.number_format == "@"
    assert topic_title_cell.alignment.wrap_text is True
    assert (
        worksheet.column_dimensions[topic_title_cell.column_letter].width >= 60
    )
...
def test_cluster_admission_blocks_low_confidence_cluster_before_downstream_models() -> None:
    class LowConfidenceClusterMimo:
        ...
        def classify_topic_stage(self, *_args, **_kwargs):
            self.downstream_calls.append("classify_topic_stage")
            raise AssertionError("低置信聚类不得进入知识分类和沉淀价值判断")

        def label_topic(self, *_args, **_kwargs):
            self.downstream_calls.append("label_topic")
            raise AssertionError("低置信聚类不得进入知识转写")

        def review_topic(self, *_args, **_kwargs):
            self.downstream_calls.append("review_topic")
            raise AssertionError("低置信聚类不得进入内容初审")

    clustering_meta: dict[str, object] = {}
    client = LowConfidenceClusterMimo()
    topics, mapping, gaps, pending = build_topic_review_rows(
        [
            {
                "数据ID": record_id,
    ...
    assert not gaps
    assert not pending
    assert {topic["主题样本数"] for topic in topics} == {2}
    assert set(topics[0]["主题工单ID"].splitlines()) == {
        "ADMISSION-LOW-001",
        "ADMISSION-LOW-002",
    }
    assert all(
        topic["主题状态"] == "provisional_multi_member_review_pending"
        for topic in topics
    )
    assert all(topic["聚类准入状态"] == "暂定多成员主题候选" for topic in topics)
    assert all(topic["聚类准入置信度"] == pytest.approx(0.68) for topic in topics)
    assert all(topic["是否重点复核"] == "是" for topic in topics)
    assert all("低于自动放行阈值" in topic["聚类准入原因"] for topic in topics)
    assert all("强制人工价值复核" in topic["聚类准入原因"] for topic in topics)
    assert client.downstream_calls == []
    ...
    assert clustering_meta["provisional_same_boundary_coalesced"] == 1
...
def test_cluster_admission_blocks_model_review_request_even_when_confidence_is_high() -> None:
    ...
    client = ReviewRequiredClusterMimo()
    topics, mapping, gaps, pending = build_topic_review_rows(
        [
            {
                "数据ID": record_id,
    ...
    assert not pending
    assert topics[0]["主题样本数"] == 2
    assert all(topic["主题状态"] == "provisional_multi_member_review_pending" for topic in topics)
    assert all(topic["聚类准入状态"] == "暂定多成员主题候选" for topic in topics)
    assert all(topic["聚类准入置信度"] == pytest.approx(0.95) for topic in topics)
...
def test_cluster_admission_blocks_direct_mimo_rule_fallback_before_topic_stage(
    ...
    assert "强制人工价值复核" in topics[0]["聚类准入原因"]
    assert client.downstream_calls == []
    assert clustering_meta["effective_mode"] == "rule"
    assert clustering_meta["cluster_admission_pending_topics"] == 0
    assert clustering_meta["cluster_admission_provisional_topics"] == 1
    assert clustering_meta["cluster_admission_provisional_candidates"] == 1
...
def test_direct_mimo_auto_merges_same_object_same_phenomenon_singletons() -> None:
    ...
    assert not gaps
    assert not pending
    assert clustering_meta["direct_reconcile_approved"] == 0
    assert clustering_meta["direct_reconcile_rule_approved"] == 0
...
def test_direct_cluster_guard_rejects_cross_component_same_standard_family(
    ...
    assert workflow_module._direct_cluster_hard_conflict_reason(rows) == (
        "判定对象不同，绝对不能自动聚类合并"
    )
...
def test_rule_fallback_organizes_human_judgment_and_keeps_missing_standard_blank() -> None:
    ...
    assert "人工确认闭合瞬间单次异响是否属于转轴异响" in content
...
def test_source_fact_keeps_exploratory_first_turn_for_audit_but_not_intent() -> None:
    fact = workflow_module._topic_source_fact(
        {
            "数据ID": "FIRST-TURN-UNCONFIRMED",
            "产品类型": "手机",
            "聊天内容": "26/09/03 10:00:01 工程师：屏幕色斑怎么判？",
        },
        1,
    ...
    assert fact["conversation_excerpt"] == (
        "26/09/03 10:00:01 工程师：屏幕色斑怎么判？"
    )
    assert fact["intent_evidence_excerpt"] == ""
    assert fact["first_turn_candidate"] == (
        "26/09/03 10:00:01 工程师：屏幕色斑怎么判？"
    )
    assert fact["first_turn_role"] == "exploratory_unconfirmed"
...
def test_memory_and_storage_brand_targets_stay_separate_from_huawei_combined_guard() -> None:
    memory_targets = {
        build_clustering_fingerprint(
            product_category="笔记本",
            subject=subject,
            normalized_issue=issue,
            judgment_target="确认内存是否为品牌认证配件",
        ).query_target
        for subject, issue in (
            ("内存", "内存是否为品牌产品"),
            ("内存", "R9000P 内存品牌、不拆机认证标准查询"),
        )
    }
    storage_targets = {
        build_clustering_fingerprint(
            product_category="笔记本",
            subject="硬盘",
            normalized_issue="硬盘是否为品牌件",
            judgment_target="确认硬盘是否为品牌认证配件",
        ).query_target
    }
    huawei_targets = {
        build_clustering_fingerprint(
            ...
            subject="内存和硬盘",
            normalized_issue="华为 MateBook 内存和硬盘是否为品牌件",
            judgment_target="确认华为品牌机型的内存硬盘特殊品牌规则",
        ).query_target
        for product_category in ("笔记本", "笔记本电脑")
    }

    assert memory_targets == {"memory_brand"}
    assert storage_targets == {"storage_brand"}
    assert huawei_targets == {"memory_storage_brand_huawei_special"}


def test_huawei_memory_only_brand_query_uses_general_memory_brand_target() -> None:
    fingerprint = build_clustering_fingerprint(
        product_category="笔记本",
        subject="内存",
        phenomenon="是否为品牌件",
        normalized_issue="笔记本内存是否为品牌件",
        judgment_target="确认内存品牌属性",
        resolution_mode="按质检选项判定",
        conversation="华为 MateBook 14 2024 的内存是品牌还是三方？",
    )

    assert fingerprint.query_target == "memory_brand"


def test_direct_reconcile_keeps_memory_and_storage_brand_queries_separate() -> None:
    def row(
        record_id: str,
        subject: str,
        issue: str,
        ...
        return {
            ...
            "核心问题": issue,
            "判定目标": judgment_target,
            "解题方式": "通过设备信息或验机工具核对品牌信息",
            "主标准路径": f"{subject}品牌属性查询",
            "_原子平台": "通用",
            "_原子品牌": "通用",
            "_原子机型范围": "通用",
            "_原子阈值例外": "无明确阈值",
    ...
    memory = row(
        "NOTEBOOK-MEMORY-BRAND",
        "内存",
        "笔记本内存是否为品牌件",
        "确认内存品牌属性",
    )
    storage = row(
        "NOTEBOOK-STORAGE-BRAND",
        "硬盘",
        "笔记本硬盘是否为品牌件",
        "确认硬盘品牌属性",
    )
    ...
    assert (
        workflow_module._direct_reconcile_fingerprint(memory).query_target
        == "memory_brand"
    )
    assert (
        workflow_module._direct_reconcile_fingerprint(storage).query_target
        == "storage_brand"
    )
    assert workflow_module._direct_reconcile_has_hard_conflict(memory, [storage])
    assert workflow_module._direct_cluster_hard_conflict_reason([memory, storage]) == (
        "可信业务目标不同，不能保留在同一主题簇"
    )


def test_waterproof_indicator_discoloration_branches_merge_only_for_manual_review() -> None:
    def row(
        record_id: str,
        subject: str,
        phenomenon: str,
    ...
    reviewer = SimpleNamespace(config=SimpleNamespace(model="offline-waterproof-test"))
    meta: dict[str, object] = {}

    reconciled = workflow_module._reconcile_direct_topic_groups(
        [
            (("direct_mimo", "手机", str(index)), [item])
    ...
    assert len(reconciled[0][1]) == 3
    topic = workflow_module._cluster_only_topic_row(
        "TOP-WATERPROOF-INDICATOR",
        ("direct_mimo", "手机"),
        reconciled[0][1],
        {"status": "已自动放行", "confidence": 0.95, "reason": "test"},
    )

    assert topic["聚类主题"] == "手机防水标变色如何判定（卡槽位置与证据分支）"
    assert topic["聚类准入状态"] == "待人工聚类复核"
    assert topic["是否重点复核"] == "是"
    assert "防水标位置或证据状态不同" in topic["聚类准入原因"]


def test_special_post_cluster_consolidation_merges_waterproof_branches_and_marks_review() -> None:
    def row(record_id: str, subject: str, phenomenon: str) -> dict[str, object]:
        return {
            "数据ID": record_id,
            "工单ID": record_id,
    ...
    groups = [
        (("direct_mimo", "one"), [row("WATERPROOF-ONE", "卡槽防水标", "变红")]),
        (("direct_mimo", "two"), [row("WATERPROOF-TWO", "防水标", "局部变色")]),
        (("direct_mimo", "three"), [row("WATERPROOF-THREE", "防水标签", "变红")]),
    ]
    meta = {"direct_reconcile_approved": 0, "direct_reconcile_rule_approved": 0}

    consolidated = workflow_module._consolidate_direct_special_topic_groups(
        groups,
        meta,
    )

    assert len(consolidated) == 1
    assert len(consolidated[0][1]) == 3
    assert meta["direct_reconcile_rule_approved"] == 2
    assert all(row["_聚类需要复核"] for row in consolidated[0][1])


def test_cluster_only_headphone_find_titles_are_clear_and_keep_business_outcomes_separate() -> None:
    common = {
        "产品类型": "耳机和耳麦",
        "回收业务层级": "自营回收",
        "模型主题一级分类": "功能问题",
    ...
    abnormal = workflow_module._cluster_only_topic_row(
        "TOP-HEADPHONE-FIND-ABNORMAL",
        ("direct_mimo", "耳机"),
        [
            {
                **common,
                ...
                "核心问题": "耳机查找功能异常是否可以回收",
                "判定目标": "确认查找功能异常是否影响回收",
                "_聚类主题标题": "耳机和耳麦定位查找功能查找功能异常如何判定",
            }
        ],
    )
    support = workflow_module._cluster_only_topic_row(
        "TOP-HEADPHONE-FIND-SUPPORT",
        ("direct_mimo", "耳机"),
        [
            {
                **common,
                ...
                "核心问题": "耳机是否具备定位查找功能",
                "判定目标": "确认耳机是否支持定位查找功能",
                "_聚类主题标题": "耳机和耳麦定位查找功能是否具备定位查找功能",
            }
        ],
    )

    assert abnormal["聚类主题"] == "耳机查找功能异常是否影响回收"
    assert support["聚类主题"] == "耳机是否支持定位查找功能"
    assert abnormal["聚类主题"] != support["聚类主题"]
    assert "查找功能查找功能" not in abnormal["聚类主题"]


def test_same_source_duplicate_headphone_find_atoms_can_merge_but_distinct_statuses_cannot() -> None:
    common = {
        "数据ID": "AIRPODS-DUPLICATE-FIND",
        "工单ID": "AIRPODS-DUPLICATE-FIND",
        "产品类型": "耳机",
        "对象/部位": "查找功能",
        "核心问题": "AirPods 查找功能异常是否影响回收",
        "异常现象": "查找功能异常",
        "判定目标": "确认查找功能异常是否影响回收",
        "解题方式": "按查找功能标准核验",
        "历史实际回复": "查找功能异常时可以正常回收。",
    }
    duplicate_a = {**common, "_原子知识ID": "AIRPODS-DUP-U1"}
    duplicate_b = {**common, "_原子知识ID": "AIRPODS-DUP-U2", "对象/部位": "配件"}
    account_bound = {
        **common,
        "_原子知识ID": "AIRPODS-DUP-U3",
        "异常现象": "查找功能已绑定账户",
        "历史实际回复": "已绑定他人账户，不回收。",
    }
    missing_item = {
        **common,
        "_原子知识ID": "AIRPODS-DUP-U4",
        "对象/部位": "物品完整性",
        "核心问题": "AirPods 少一个底部配件是否影响回收",
        "异常现象": "物品缺失",
        "判定目标": "确认缺失物品是否影响回收",
        "解题方式": "按物品完整性标准核验",
    }

    assert not workflow_module._same_source_different_atomic_conflict(
        [duplicate_a, duplicate_b]
    )
    assert workflow_module._same_source_different_atomic_conflict(
        [duplicate_a, account_bound]
    )
    assert workflow_module._same_source_different_atomic_conflict(
        [duplicate_a, missing_item]
    )


def test_special_post_cluster_consolidation_deduplicates_same_find_status_only() -> None:
    def row(atomic_id: str, phenomenon: str, reply: str) -> dict[str, object]:
        return {
            "数据ID": "AIRPODS-POST-DEDUPE",
            "工单ID": "AIRPODS-POST-DEDUPE",
            "_原子知识ID": atomic_id,
            "回收业务层级": "自营回收",
            "产品类型": "耳机",
            "对象/部位": "查找功能",
            "异常现象": phenomenon,
            "核心问题": "AirPods 查找功能如何判定",
            "判定目标": "确认查找功能状态是否影响回收",
            "解题方式": "按查找功能标准核验",
            "历史实际回复": reply,
        }

    abnormal_a = row("FIND-U1", "查找功能异常", "异常时可以回收。")
    abnormal_b = row("FIND-U2", "查找功能异常", "异常时可以回收。")
    bound = row("FIND-U3", "查找功能已绑定账户", "绑定账户不回收。")
    missing_item = {
        **row("FIND-U4", "物品缺失", "查找功能异常时可以回收。"),
        "对象/部位": "物品完整性",
        "核心问题": "AirPods 少一个底部配件是否影响回收",
        "判定目标": "确认缺失物品是否影响回收",
        "解题方式": "按物品完整性标准核验",
    ...
    consolidated = workflow_module._consolidate_direct_special_topic_groups(
        [
            (("direct_mimo", "abnormal-a"), [abnormal_a]),
            (("direct_mimo", "abnormal-b"), [abnormal_b]),
            (("direct_mimo", "bound"), [bound]),
            (("direct_mimo", "missing-item"), [missing_item]),
        ],
        meta,
    )

    assert len(consolidated) == 3
    assert sorted(len(rows) for _key, rows in consolidated) == [1, 1, 2]
    assert meta["direct_reconcile_rule_approved"] == 1


def test_direct_reconcile_fingerprint_keeps_huawei_memory_brand_topic_separate() -> None:
    common = {
        "产品类型": "笔记本",
        "模型主题一级分类": "信息查询",
        "问题意图": "信息查询",
    ...
    dell = {
        **common,
        "机型": "戴尔 G15 5511",
        "聊天内容": "这个是第三方硬盘吗？截图显示三星。",
        "核心问题": "笔记本硬盘品牌是否为第三方",
    }
    huawei = {
        **common,
        "机型": "华为 MateBook D 14 2024",
        "聊天内容": "华为 MateBook 的内存和硬盘是不是品牌件？",
        # 真实原子行会用 MiMo 的泛化 normalized_issue 覆盖“核心问题”，
        # 但来源机型和聊天仍保留华为证据；二次归并必须读取这些来源字段。
        "核心问题": "笔记本内存和硬盘品牌属性查询",
    }

    dell_fingerprint = workflow_module._direct_reconcile_fingerprint(dell)
    huawei_fingerprint = workflow_module._direct_reconcile_fingerprint(huawei)

    assert dell_fingerprint.query_target == "storage_brand"
    assert huawei_fingerprint.query_target == "memory_storage_brand_huawei_special"
    assert dell_fingerprint.query_target != huawei_fingerprint.query_target


def test_topic_boundary_key_uses_source_huawei_model_context() -> None:
    row = {
        "产品类型": "笔记本",
        "模型主题一级分类": "信息查询",
        "问题意图": "信息查询",
    ...
    boundary = workflow_module._topic_boundary_key(row)

    assert boundary is not None
    assert "memory_storage_brand_huawei_special" in boundary


def test_direct_atomic_bucket_uses_preserved_source_huawei_model_context() -> None:
    unit = {
        "product_category": "笔记本",
        "category_l1": "信息查询",
        "intent": "信息查询",
        "subject": "硬盘",
        "phenomenon": "品牌识别",
        "normalized_issue": "笔记本内存和硬盘品牌属性查询",
        "judgment_target": "确认硬盘品牌属性",
        "resolution_mode": "告知平台标准，无需识别品牌",
        "standard_path": "待确认",
        "evidence_summary": "现场截图用于确认硬盘品牌。",
        "source_model": "华为 MateBook D 14 2024",
        "source_conversation": "内存和硬盘是不是品牌件？",
        "platform": "通用",
        "brand": "通用",
        "model_scope": "通用",
    }

    bucket = workflow_module._direct_atomic_bucket_key(unit)

    assert "memory_storage_brand_huawei_special" in bucket


def test_headphone_find_recovery_outcomes_are_hard_merge_conflict() -> None:
    recoverable = {
        "数据ID": "AIRPODS-FIND-RECOVERABLE",
        "工单ID": "AIRPODS-FIND-RECOVERABLE",
        "_原子知识ID": "AIRPODS-FIND-RECOVERABLE-U1",
        "产品类型": "耳机",
        "模型主题一级分类": "功能问题",
        "模型主题二级分类": "查找功能",
        "问题意图": "回收资格判定",
        "对象/部位": "查找功能",
        "异常现象": "查找功能异常",
        "核心问题": "AirPods 查找功能异常是否可以回收",
        "判定目标": "确认查找功能异常时的回收资格",
        "解题方式": "核对查找功能状态和回收口径",
        "主标准路径": "耳机使用与账号标准",
        "历史实际回复": "查找功能异常的情况可以正常回收，请正常下单即可。",
        "_原子平台": "通用",
        "_原子品牌": "Apple",
        "_原子机型范围": "AirPods Pro",
    }
    account_locked = {
        **recoverable,
        "数据ID": "AIRPODS-FIND-LOCKED",
        "工单ID": "AIRPODS-FIND-LOCKED",
        "_原子知识ID": "AIRPODS-FIND-LOCKED-U1",
        "异常现象": "已连接他人的 Apple 账户，激活锁",
        "核心问题": "AirPods Pro 连接他人的 Apple 账户激活锁如何处理",
        "判定目标": "确认激活锁设备是否可回收",
        "历史实际回复": "设备已绑定他人的 Apple 账户，属于激活锁，不回收。",
    }

    assert workflow_module._direct_cluster_hard_conflict_reason(
        [recoverable, account_locked]
    ) == "耳机查找功能标准状态不同，不能自动聚类合并"
    assert not workflow_module._headphone_find_recovery_outcome_conflict(
        [recoverable, account_locked]
    )
    assert workflow_module._direct_reconcile_has_hard_conflict(
        recoverable,
    ...
    assert workflow_module._has_topic_merge_conflict(recoverable, account_locked)


def test_headphone_find_standard_states_are_distinguished_without_recovery_shortcut() -> None:
    def row(phenomenon: str, reply: str) -> dict[str, object]:
        return {
            "产品类型": "耳机",
            "对象/部位": "查找功能",
            "异常现象": phenomenon,
    ...
    normal = row(
        "查找功能正常",
        "查找 App 内有查找功能且可以正常使用。",
    )
    abnormal_recoverable = row(
        "查找功能异常",
        "查找网络功能缺失，但当前口径说明该异常情况可以正常回收。",
    )
    bound_account = row(
        "查找功能已绑定账户",
        "配对时提示此物品的物主将可查看其位置，属于不回收类型。",
    )
    unsupported = row(
        "其他版本查找功能不支持",
        "外版华为耳机在国内不支持使用查找功能。",
    ...
    assert workflow_module._headphone_find_standard_status(normal) == "normal"
    assert (
        workflow_module._headphone_find_standard_status(abnormal_recoverable)
        == "abnormal"
    )
    assert (
        workflow_module._headphone_find_standard_status(bound_account)
        == "bound_account"
    )
    assert (
        workflow_module._headphone_find_standard_status(unsupported)
        == "unsupported_version"
    )
    assert workflow_module._headphone_find_recovery_outcome(abnormal_recoverable) == ""
    assert workflow_module._headphone_find_recovery_outcome_conflict(
        [abnormal_recoverable, bound_account]
    ) is False
    assert workflow_module._direct_cluster_hard_conflict_reason(
        [normal, abnormal_recoverable]
    ) == "耳机查找功能标准状态不同，不能自动聚类合并"
    assert workflow_module._direct_cluster_hard_conflict_reason(
        [abnormal_recoverable, bound_account]
    ) == "耳机查找功能标准状态不同，不能自动聚类合并"
    assert workflow_module._direct_cluster_hard_conflict_reason(
        [bound_account, unsupported]
    ) == "耳机查找功能标准状态不同，不能自动聚类合并"


def test_headphone_find_brand_specific_missing_entry_keeps_huawei_bound_and_android_abnormal() -> None:
    huawei = {
        "产品类型": "耳机",
        "品牌": "华为",
        "产品型号": "FreeBuds Pro4",
        "对象/部位": "查找功能",
        "异常现象": "华为耳机查找功能入口缺失",
        "核心问题": "华为支持查找定位的耳机入口缺失如何判定",
        "历史实际回复": "安装音频管家插件并更新系统固件后，查找入口仍缺失。",
    }
    vivo = {
        "产品类型": "耳机",
        "品牌": "vivo",
        "对象/部位": "查找功能",
        "异常现象": "查找网络功能缺失，仅剩查找耳机",
        "核心问题": "vivo 耳机查找网络功能缺失如何判定",
    }

    assert workflow_module._headphone_find_standard_status(huawei) == "bound_account"
    assert workflow_module._headphone_find_standard_status(vivo) == "abnormal"
    assert workflow_module._direct_cluster_hard_conflict_reason([huawei, vivo]) == (
        "耳机查找功能标准状态不同，不能自动聚类合并"
    )


def test_headphone_find_standard_status_requires_complete_brand_evidence() -> None:
    huawei_generic_missing_entry = {
        "产品类型": "耳机",
        "品牌": "华为",
        "异常现象": "查找功能入口缺失",
        "核心问题": "华为耳机查找功能入口缺失如何判定",
    }
    huawei_supported_missing_entry = {
        "产品类型": "耳机",
        "品牌": "华为",
        "产品型号": "FreeBuds Pro4",
        "异常现象": "查找功能入口缺失",
        "历史实际回复": "安装音频管家插件或更新系统固件后，查找入口仍缺失。",
    }
    apple_incomplete_normal = {
        "产品类型": "耳机",
        "品牌": "Apple",
        "异常现象": "查找功能检查",
        "历史实际回复": "查找 App 内有查找功能。",
    }
    vivo_incomplete_normal = {
        "产品类型": "耳机",
        "品牌": "vivo",
        "异常现象": "查找功能检查",
        "历史实际回复": "查找网络可切换打开和关闭。",
    }
    huawei_incomplete_unsupported = {
        "产品类型": "耳机",
        "品牌": "华为",
        "异常现象": "查找功能检查",
        "历史实际回复": "外版有定位功能。",
    ...
    huawei_unsupported = {
        ...
        "品牌": "华为",
        "异常现象": "查找功能检查",
        "历史实际回复": "外版有定位功能，在国内不支持使用查找功能。",
    }

    assert workflow_module._headphone_find_standard_status(
        huawei_generic_missing_entry
    ) == ""
    assert workflow_module._headphone_find_standard_status(
        huawei_supported_missing_entry
    ) == "bound_account"
    assert workflow_module._headphone_find_standard_status(apple_incomplete_normal) == ""
    assert workflow_module._headphone_find_standard_status(vivo_incomplete_normal) == ""
    assert (
        workflow_module._headphone_find_standard_status(
            huawei_incomplete_unsupported
        )
        == ""
    )
    assert workflow_module._headphone_find_standard_status(huawei_unsupported) == (
        "unsupported_version"
    )


def test_headphone_find_mixed_bound_account_and_recoverable_evidence_forces_case_review() -> None:
    topic_payload = {
        "product_categories": ["耳机"],
        "normalized_issues": ["AirPods 查找功能已绑定账户如何判定"],
        "upstream_core_problems": [],
        "upstream_judgment_conclusions": ["已绑定他人 Apple 账户，不回收。"],
        "phenomena": ["查找功能已绑定账户"],
        "historical_replies": ["历史回复同时写有可以正常回收。"],
        "conversation_evidence": [],
        "representative_human_judgments": [],
        "evidence_summaries": [],
    }

    assert workflow_module._topic_is_headphone_account_lock_case(topic_payload)


def test_headphone_find_atomic_topic_uses_source_standard_status_over_mixed_model_phenomenon() -> None:
    source_row = {
        "产品类型": "耳机",
        "聊天内容": (
            "回收 AirPods Pro 时提示已连接其他人的 Apple 账户，"
            "查找 App 仅有播放声音和路线，没有查找功能。"
        ),
        "判定结论": (
            "旧口径曾标注为查找功能异常；但该设备已连接他人 Apple 账户，"
            "查找功能已绑定账户，属于不回收类型。"
        ),
        "历史实际回复": "存在激活锁，不回收。",
    }
    model_topic = {
        "product_category": "耳机",
        "subject": "查找功能",
        "phenomenon": "连接他人 Apple 账户（激活锁）且查找功能异常",
        "normalized_issue": "AirPods 查找功能异常如何判定",
    }

    normalized = workflow_module._normalize_headphone_find_atomic_topic(
        source_row,
        model_topic,
    )

    assert normalized["phenomenon"] == "查找功能已绑定账户"
    assert normalized["_耳机查找功能标准状态"] == "bound_account"
    assert normalized["_耳机查找功能模型现象"] == (
        "连接他人 Apple 账户（激活锁）且查找功能异常"
    )
    rule_match = workflow_module._headphone_find_standard_rule_match(source_row)
    assert rule_match is not None
    assert rule_match.rule_id == "headphones-use-and-connection"
    assert rule_match.phenomenon_value == "查找功能已绑定账户"


def test_headphone_find_source_status_does_not_relabel_unrelated_atomic_topic() -> None:
    source_row = {
        "产品类型": "耳机",
        "聊天内容": "查找功能异常，可以回收；另外少一个底部配件。",
        "判定结论": "查找功能异常不影响回收。",
    }
    missing_item = {
        "product_category": "耳机",
        "subject": "物品完整性",
        "phenomenon": "物品缺失",
        "normalized_issue": "AirPods 少一个底部配件是否影响回收",
        "judgment_target": "确认缺失物品是否影响回收",
        "resolution_mode": "按物品完整性标准核验",
    }
    ...
    assert normalized == missing_item
    assert "_耳机查找功能标准状态" not in normalized


def test_headphone_account_lock_case_is_not_transcribed_as_knowledge() -> None:
    class AccountLockMimoMustNotRun:
        config = SimpleNamespace(model="account-lock-test")

        def __init__(self) -> None:
            self.calls: list[str] = []

        def classify_topic_stage(self, *_args, **_kwargs):
            self.calls.append("classify_topic_stage")
            raise AssertionError("激活锁案例不得调用主题分类模型")

        def label_topic(self, *_args, **_kwargs):
            self.calls.append("label_topic")
            raise AssertionError("激活锁案例不得调用知识转写模型")

    rows = [
        {
            "数据ID": "AIRPODS-ACCOUNT-LOCK-001",
            "工单ID": "AIRPODS-ACCOUNT-LOCK-001",
            "聊天内容": (
                "回收 Apple AirPods Pro 时，设备提示已连接其他人的 Apple 账户，"
                "查找功能菜单也缺失，应该如何处理？"
            ),
            "核心问题": "AirPods Pro 连接他人的 Apple 账户激活锁如何处理",
            "原始判定结论": "该设备存在他人 Apple 账户激活锁，按当前案例不回收。",
            "判定结论": "该设备存在他人 Apple 账户激活锁，按当前案例不回收。",
            "历史实际回复": (
                "设备已绑定他人的 Apple 账户，属于激活锁，不回收；"
                "但历史记录也出现过可以正常回收的相反回复。"
            ),
            "产品类型": "耳机",
            "模型主题一级分类": "功能问题",
            "模型主题二级分类": "查找功能",
            ...
            "异常现象": "已连接他人的 Apple 账户，激活锁",
            "解题方式": "核对账号绑定状态",
            "语义标注依据": "来源图片和会话仅说明本台设备已被他人 Apple 账户锁定。",
        }
    ]
    mimo = AccountLockMimoMustNotRun()
    standard_calls: list[str] = []

    def standard_retriever(
        topic_id: str,
        _rows: list[dict[str, object]],
        _query: dict[str, object],
    ) -> tuple[list[tuple[StandardCatalogItem, float]], dict[str, object]]:
        standard_calls.append(topic_id)
        raise AssertionError("激活锁案例不得检索标准")
    ...
    topics, _mapping, gaps, pending = build_topic_review_rows(
        ...
        clustering_mode="rule",
        use_standard_references=True,
        topic_standard_retriever=standard_retriever,
    )
    ...
    assert topics[0]["主题问题分类"] == "案例解析"
    assert topics[0]["主题沉淀价值"] == "不值得沉淀"
    assert topics[0]["主题状态"] == "case_review_pending"
    assert topics[0]["主题转写状态"] == "skipped_account_lock_case"
    assert "未生成知识草稿" in topics[0]["知识内容"]
    assert "不回收" not in topics[0]["知识内容"]
...
def test_clean_headphone_bound_account_standard_is_not_forced_into_case_only_guard() -> None:
    rows = [
        {
            "数据ID": "AIRPODS-ACCOUNT-BOUND-STANDARD-001",
            "工单ID": "AIRPODS-ACCOUNT-BOUND-STANDARD-001",
            "聊天内容": (
                "苹果耳机配对时提示此物品的物主将可查看其位置，"
                "请按查找功能已绑定账户口径处理。"
            ),
            "核心问题": "苹果耳机查找功能已绑定账户如何判定",
            "原始判定结论": "提示物主可查看位置，属于查找功能已绑定账户，不回收。",
            "判定结论": "提示物主可查看位置，属于查找功能已绑定账户，不回收。",
            "历史实际回复": "查找功能已绑定账户，按不回收类型处理。",
            "产品类型": "耳机",
            "模型主题一级分类": "功能问题",
            "模型主题二级分类": "查找功能",
            "问题意图": "标准判定",
