...
当前是**CZ 标准驱动转写模式（聚类不读取标准）**：

- 聚类主证据仍是完整脱敏会话、历史实际回复和脱敏案例图；标准不参与自动聚类。
- 配置`KB_RETRIEVAL_API_KEY`后，所有通过聚类准入的主题都会先按需检索 CZ 中已发布的`headquarters_standard`总部标准；主题价值标签只决定人工复核优先级，不再阻止候选转写。
- 总部标准未命中或检索失败时，转写和内容初审继续匹配本地已编译的正式质检标准；总部标准命中优先于本地标准。实际作为转写依据的总部或本地标准均保留在`关联标准项`；经验补充候选保持为空。
- 两类标准均无依据或与案例证据冲突时，才按可追溯案例事实生成经验补充候选并进入人工价值复核；不得自动补写标准、阈值或确定性结论。
- 已有标准关联、来源版本或正文标准引用不删除。
...
5. 完整自动化只自动合并置信度不低于`0.75`且无风险标记的聚类。不同回收业务层级或
   不同产品品类（含未识别）进入`pending_cluster_rows`，不继续生成候选；原子低置信、
   聚类低置信、规则降级、调用失败、冲突拆分或模型要求复核时，每个原子问题生成暂定
   单主题候选，继续转写和初审并强制进入人工价值复核，不参与历史主题自动归并。
...
所有通过聚类准入的主题都会进入知识转写；“是否值得沉淀”作为人工价值复核的输入，不再阻止草稿生成。总部标准未命中时会继续匹配本地已编译质检标准；两类标准均未命中时才标记为经验补充候选，必须人工确认后才能送审。
...
自营回收的`适用范围`只能使用上述12个产品品类本身，例如`手机`、`笔记本`、`平板电脑`；
明确但未纳入12项的聚合回收产品保留来源名称，例如`运动相机`、`组装机`。空值、“机型”
或“待确定/待确认”等占位值才视为未识别。来源事实明确限制品牌或具体机型时，分别填写
`适用品牌`、`适用机型`；找不到来源时必须留空，不得推测。
...
- 把本地标准文件检索作为默认主链路；当前默认优先读取 CZ 已发布的`headquarters_standard`总部标准，本地已编译质检标准只在 CZ 未命中或检索失败时作为转写兜底。两类标准均只用于转写和人工审核。Answer Hub 输出的`business_accumulation`候选只进入人工审核，不作为转写输入。
