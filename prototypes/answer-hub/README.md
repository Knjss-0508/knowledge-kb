...
自营回收按固定12品类配置处理手机、平板电脑、智能手表、耳机/耳麦、笔记本、游戏机、游戏卡带、单电/微单机身、单反机身、相机镜头、手写笔和学习机；单条高质量原子问题也可以形成待审核主题。明确但不在这12项中的产品归入聚合回收并保留原名称；仅空值、“机型”“待确定/待确认”等占位值视为未识别。品牌和机型使用独立字段：

聚类硬边界采用 `回收业务层级 -> 产品品类 -> 业务主题`。现有质检规则属于“自营回收”；
“聚合回收”已预留为并列业务层级，使用来源的明确产品名称作为品类，但专属质检口径
尚未配置，因此当前聚合回收记录只做同名产品的层级隔离并强制人工优先审核。自营回收
与聚合回收绝对不能聚类到一起。
...
→ 原子问题拆分
→ 纯大模型 1～N 聚类
→ 高置信聚类准入并自动合并；低置信和风险聚类生成暂定单主题候选
...
- 产品类型或业务层级冲突、未识别时写入`pending_cluster_rows`，禁止自动合并和候选转写；
  低置信、规则降级、调用失败、冲突拆分或模型要求复核时，每个原子问题生成暂定单主题
  候选，继续分类、转写和初审，但强制人工价值复核且不参与历史主题自动归并。
...
- 聚类阶段不读取标准。所有通过聚类准入或暂定候选的主题都会在转写阶段先读取 CZ 已发布的`headquarters_standard`总部标准；若未命中，再匹配本地已编译的正式质检标准；两者都无依据时才生成基于来源事实的经验补充候选。主题价值标签只决定人工复核优先级，不阻止转写。实际作为转写依据的总部或本地标准写入`关联标准项`；经验补充候选保持为空，并强制人工价值复核；不得编造标准、阈值或确定性结论。Answer Hub 转写出的候选知识属于`business_accumulation`，只进入 CZ 人工审核，不会被反向读取。已有标准关联不覆盖。
- Qwen3 Embedding 对批量导入执行重复拦截；自动化批量场景还必须具备标题或正文的有效文本重合证据，避免同品类、同回复模板的不同知识被整批误拦截。
- 服务端会统一检查知识ID、主标题、知识内容、推荐回复、知识分类、适用范围和关键词；标准关联会随候选进入人工价值复核，仍不绕过 Qwen3 查重、CZ 终审或人工发布。
...
`第二部分数据 -> 数据预处理 -> 聚类 -> CZ已生效业务沉淀标准驱动转写 -> 人工复核 -> Qwen3查重拦截 -> CZ待审核 -> 发布`
...
配置`KB_RETRIEVAL_API_KEY`后，所有通过聚类准入或暂定候选的主题会按需读取 CZ 已发布的`headquarters_standard`总部标准。标准只用于转写和人工审核，不进入自动聚类；总部标准未命中或检索失败时，会继续匹配本地`--standards`正式质检标准；两者均无依据或与案例冲突时，才按来源事实生成经验补充候选并进入人工价值复核。经验补充候选不得编造标准条款、阈值或平台规则，`关联标准项`必须为空；实际使用的总部或本地标准必须保留可追溯引用。Answer Hub 产生的`business_accumulation`候选只提交 CZ 人工审核，不能作为读取标准。
...
KB_RETRIEVAL_API_KEY=通过安全环境变量提供，仅用于读取 CZ 已发布总部标准
```
